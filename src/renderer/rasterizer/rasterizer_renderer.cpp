...
void cg::renderer::rasterization_renderer::render()
{
	...
	rasterizer->clear_render_target({150, 150, 150});
	auto finish = std::chrono::high_resolution_clock::now();

	std::chrono::duration<float, std::milli> clearing_time = finish - start;
	std::cout<< "Clearing time " << clearing_time.count() << " ms\n";
	...
	std::chrono::duration<float, std::milli> rendering_time = finish - start;
	std::cout<< "Rendering time " << rendering_time.count() <<  " ms\n";
