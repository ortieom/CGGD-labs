...
namespace cg::renderer
{
	...
	inline void rasterizer<VB, RT>::draw(size_t num_vertexes, size_t vertex_offset)
	{
		...
		while (vertex_id < vertex_offset + num_vertexes)
		{
			...
			int2 bounding_box_begin = clamp(min_vertex, min_border, max_border);
			int2 max_vertex = max(vertex_a, max(vertex_b, vertex_c));
			int2 bounding_box_end = clamp(max_vertex, min_border, max_border);
			...
			for (int x = bounding_box_begin.x; x <= bounding_box_end.x; x++)
			{
				for (int y = bounding_box_begin.y; y <= bounding_box_end.y; y++)
				{
					...
					if (edge0 >= 0 && edge1 >= 0 && edge2 >= 0)
					{
						...
						{
							auto pixel_res = pixel_shader(vertices[0], 0.f);
							render_target->item(u_x, u_y) = RT::from_color(pixel_res);
							if (depth_buffer)
